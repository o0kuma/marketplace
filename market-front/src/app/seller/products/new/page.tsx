...
import type { OptionGroupInput, ProductVariantInput } from "@/types/product";
...
export default function NewProductPage() {
  ...
  function addOptionValue(groupIndex: number) {
    setOptionGroups((prev) => {
      const next = prev.map((g, i) =>
        i === groupIndex
          ? { ...g, values: [...g.values, { name: "", sortOrder: g.values.length }] }
          : g
      );
      return next;
    });
  ...
  function updateOptionValue(groupIndex: number, valueIndex: number, name: string) {
    setOptionGroups((prev) => {
      const next = prev.map((g, i) =>
        i === groupIndex
          ? { ...g, values: g.values.map((v, j) => (j === valueIndex ? { ...v, name } : v)) }
          : g
      );
      return next;
    });
  ...
  function addVariantRow() {
    setVariants((prev) => [
      ...prev,
      {
        optionValueNames: optionGroups.map(() => ""),
        price: "",
        stockQuantity: "",
        sku: "",
      },
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const hasOptions = useOptions && optionGroups.length > 0 && optionGroups.every((g) => g.values.some((v) => v.name.trim()));
  ...
  useEffect(() => {
    ...
    else api<{ id: number; name: string }[]>("/categories").then(setCategories).catch(() => setCategories([]));
  }, [user, router]);
  ...
  return (
    <div className="py-4">
      ...
      <h1 className="section-title mt-4">상품 등록</h1>
      <form onSubmit={handleSubmit} className="card mt-6 max-w-2xl space-y-5">
        ...
        <label className="block">
          <span className="label">카테고리 (필수)</span>
          <select value={categoryId} onChange={(e) => setCategoryId(e.target.value)} required className="input-field w-full">
              ...
              {optionGroups.map((g, gi) => (
                <div key={gi} className="mb-4 rounded-lg border border-[var(--market-border)] bg-[var(--market-surface)] p-3">
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    />
                    <span className="text-zinc-500 text-sm">옵션값:</span>
                    {g.values.map((val, vi) => (
                    ...
                    <thead>
                      <tr className="bg-zinc-100">
                        {optionGroups.map((g, i) => (
                          <th key={i} className="border-b border-zinc-200 px-2 py-2 text-left font-medium">{g.name || `옵션${i + 1}`}</th>
                        ))}
                        ...
                        <th className="border-b border-zinc-200 px-2 py-2 w-16" />
                      </tr>
                    </thead>
                    <tbody>
                      {variants.map((row, ri) => (
                        <tr key={ri} className="border-b border-zinc-100">
                          {optionGroups.map((g, gi) => (
                            <td key={gi} className="px-2 py-1.5">
                              ...
                              >
                                ...
                                {g.values.filter((v) => v.name.trim()).map((v) => (
                          ...
                          <td className="px-2 py-1.5">
                            <button type="button" onClick={() => removeVariantRow(ri)} className="text-red-600 text-xs hover:underline">
                              삭제
                            </button>
                          </td>
                        </tr>
