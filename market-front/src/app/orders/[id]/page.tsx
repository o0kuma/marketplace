...
function SellerStatusSelect({
  ...
  return (
    <label className="flex flex-wrap items-center gap-2">
      <span className="text-sm font-medium text-[var(--market-text)]">다음 단계</span>
      <select
        defaultValue=""
        onChange={(e) => {
  ...
  return (
    <div>
      ...
      {returnModalType && (
        ...
        >
          <div
            className="w-full max-w-md rounded-2xl border border-[var(--market-border)] bg-[var(--market-surface)] p-6 shadow-xl"
            onClick={(e) => e.stopPropagation()}
          >
            <h2 id="return-modal-title" className="text-lg font-semibold text-[var(--market-text)]">
      ...
      {isSellerView ? (
        <Link href="/seller/orders" className="text-sm font-medium text-[var(--market-text-muted)] hover:text-[var(--market-text)]">
          ← 판매 주문 목록
        </Link>
      ) : (
        <Link href="/orders" className="text-sm font-medium text-[var(--market-text-muted)] hover:text-[var(--market-text)]">
          ← 주문 목록
        </Link>
      )}
      <div className="card mt-6">
        ...
        <section className="mt-6 border-t border-[var(--market-border)] pt-4">
          <h2 className="text-sm font-semibold text-[var(--market-text)]">배송 정보</h2>
          <p className="mt-1 text-[var(--market-text-muted)]">{order.recipientName ?? "-"}</p>
          <p className="text-sm text-[var(--market-text-muted)]">{order.recipientPhone ?? "-"}</p>
          <p className="text-sm text-[var(--market-text-muted)]">{order.recipientAddress ?? "-"}</p>
          {order.recipientCountry && order.recipientCountry !== "KR" && (
            <p className="text-sm text-[var(--market-text-muted)]">배송 국가: {order.recipientCountry}</p>
          )}
          {isSellerView && (
            <div className="mt-3 flex flex-wrap items-center gap-2">
              <span className="text-sm font-medium text-[var(--market-text)]">운송장 번호</span>
              <input
                type="text"
                value={trackingInput}
        ...
        <section className="mt-6 border-t border-[var(--market-border)] pt-4">
          ...
          <ul className="mt-2 space-y-2">
            {order.items.map((item, i) => (
              <li key={i} className="flex flex-wrap items-center justify-between gap-2 text-sm">
                <Link href={`/products/${item.productId}`} className="font-medium text-[var(--market-text)] hover:underline">
                  {item.productName}
                </Link>
          ...
          {(order.shippingFee != null && order.shippingFee > 0) || order.subtotalAmount != null ? (
            <div className="mt-3 space-y-1 text-sm text-[var(--market-text-muted)]">
          ...
          <p className="mt-2 text-lg font-semibold text-[var(--market-text)]">총 결제 금액 {order.totalAmount.toLocaleString()}원</p>
        </section>
        <section className="mt-6 border-t border-[var(--market-border)] pt-4">
          <h2 className="text-sm font-semibold text-[var(--market-text)]">반품/교환 요청</h2>
          {returnRequests.length > 0 ? (
            <ul className="mt-2 space-y-2">
              {returnRequests.map((rr) => (
                <li key={rr.id} className="rounded-lg border border-[var(--market-border)] bg-[var(--market-accent-subtle)] px-3 py-2 text-sm">
                  <span className="font-medium">{rr.type === "RETURN" ? "반품" : "교환"}</span>
                  {" · "}
                  <span className={rr.status === "REQUESTED" ? "text-amber-700" : rr.status === "APPROVED" ? "text-green-700" : rr.status === "REJECTED" ? "text-red-700" : "text-[var(--market-text-muted)]"}>
                    {rr.status === "REQUESTED" ? "요청됨" : rr.status === "APPROVED" ? "승인" : rr.status === "REJECTED" ? "거절" : "처리완료"}
                  </span>
                  {rr.reason && <p className="mt-1 text-[var(--market-text-muted)]">사유: {rr.reason}</p>}
                  {rr.sellerComment && <p className="mt-1 text-[var(--market-text-muted)]">판매자 메모: {rr.sellerComment}</p>}
            ...
            <p className="mt-1 text-sm text-[var(--market-text-muted)]">반품/교환 요청 내역이 없습니다.</p>
          )}
        ...
        <div className="mt-6 flex flex-wrap gap-3 border-t border-[var(--market-border)] pt-4">
          {isSellerView ? (
            <>
